...
class TestView(TestCase):
    def setUp(self):
        self.client = Client()
        self.user_trump = User.objects.create_user(username='trump', password='xmfjavm123')
        self.user_obama = User.objects.create_user(username='obama', password='dhqkak123')
    ...
    def navbar_test(self, soup):
        navbar = soup.nav
        self.assertIn('Song DALL-E', navbar.text)
        self.assertIn('About Me', navbar.text)

        logo_btn = navbar.find('a', text="Song DALL-E by I'm a natural")
    ...
    def test_create_post(self):
        ...
        self.client.login(username='trump', password='xmfjavm123')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        # staff인 obama가 로그인한다.
        self.client.login(username='obama', password='dhqkak123')
        response = self.client.get('/blog/create_post/')
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
    ...
    def test_update_post(self):
        ...
        self.assertEqual('Edit Post - Song DALL-E', soup.title.text)
        main_area = soup.find('div', id='main-area')
